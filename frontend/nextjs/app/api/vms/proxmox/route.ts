...
export async function GET(request: NextRequest) {
  try {
    ...
    // Call the backend API to get VMs from Proxmox nodes
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'https://cs2.drandex.org'
    const response = await fetch(`${apiUrl}/vms/proxmox`, {
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: 'Failed to fetch VMs from Proxmox nodes' },
    ...
    console.error('Error fetching VMs from Proxmox nodes:', error)
    return NextResponse.json(
      { error: 'Failed to fetch VMs from Proxmox nodes' },
      { status: 500 }
    )
