...
CREATE INDEX IF NOT EXISTS idx_farmlabs_jobs_job_id ON public.farmlabs_jobs (job_id);
CREATE INDEX IF NOT EXISTS idx_farmlabs_jobs_vm_id ON public.farmlabs_jobs (vm_id);
CREATE INDEX IF NOT EXISTS idx_farmlabs_jobs_bot_account_id ON public.farmlabs_jobs (bot_account_id);
CREATE INDEX IF NOT EXISTS idx_farmlabs_jobs_status ON public.farmlabs_jobs (status);
CREATE INDEX IF NOT EXISTS idx_farmlabs_jobs_job_type ON public.farmlabs_jobs (job_type);
CREATE INDEX IF NOT EXISTS idx_farmlabs_jobs_owner_id ON public.farmlabs_jobs (owner_id);
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE polname = 'farmlabs_jobs_user_policy' 
        AND polrelid = 'public.farmlabs_jobs'::regclass
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policy 
        WHERE polname = 'farmlabs_jobs_admin_policy' 
        AND polrelid = 'public.farmlabs_jobs'::regclass
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE tgname = 'update_farmlabs_jobs_updated_at' 
        AND tgrelid = 'public.farmlabs_jobs'::regclass
    ) THEN
